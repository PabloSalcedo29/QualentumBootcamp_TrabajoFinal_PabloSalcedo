...
def limpiar_texto(texto):
    ...
    texto = ' '.join(texto.split())  # Reducir espacios múltiples a uno solo
...
def extraer_informacion(texto):
    ...
    match_principio = re.search(r"\s*Principio\s+activo\s+es\s*(.*?)(?=\n|$)", texto, re.IGNORECASE)
...
def procesar_texto_completo(texto):
    ...
    fragmentos = dividir_texto(texto_limpio, max_tokens=256)
